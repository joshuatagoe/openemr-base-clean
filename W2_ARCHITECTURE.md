...
| 007 | **Boxes and verification come from the page, not the model.** Text layer first (pdfplumber); image-only pages and photos read by AWS Textract, one page per call; OCR also runs on a page whose text layer misses a value (handwriting on printed forms); photos rotated upright first; one matcher decides verified / unverified; one PDF/PNG/JPEG allow-list; fake OCR in CI. (§1.6) | Accepted; not built. Live evidence 2026-09-25: Textract runs in **us-east-2** (the AWS organisation's region policy denies it in us-east-1 and us-west-2); synthetic handwriting-style forms read with 0.96 word / 0.94 field recall; an image-only lab scan page in 1.2 s |
...
| 011 | **Follow-ups on documents use the Week 1 follow-up path**, seeing the chart plus pending (unfiled) document facts, always labelled "not yet verified or filed". | Accepted in full 2026-09-25: separate pending-facts tool, verifier rules, agent deployed before the module, plus a smoke test of the new briefing contract; not built |
