        ...
        for m in _SENTENCE.finditer(plan_text):
            ...
            for candidate in candidates:  # longest phrase first, e.g. "fasting lipid panel" before "lipid"
                if resolve_commitment(candidate) is not None:
                    ...
                    break
        return ModelExtractionResult(output=ModelExtractionOutput(commitments=commitments), usage=ModelUsage(provider=self.name, model=self.model, input_tokens=0, output_tokens=0, latency_ms=0))
