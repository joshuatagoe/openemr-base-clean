...
**Status (2026-09-18).** Phases 0–5 of §17 are implemented on `main`; self-hosted Langfuse and the Railway deployment are pending. Where the implementation departed from the original plan, the decision is recorded in the section concerned (§6, §7, §8, §12, §16).
...
| 5 — Hardening | Load tests at 10/50, baselines, alerts, dashboard, cost analysis, seeded-DB eval tier, docs | Phase 4 | Operability | Partial: `/ready`, `/metrics`, cost accounting, stub provider and a load baseline exist; Langfuse dashboard, Railway deployment and module-side load test pending |
